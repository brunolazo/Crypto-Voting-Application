...
async function deposit({ currency, amount }) {
    ...
    console.log(`Memorizza la tua nota: ${noteString}`)
    if (currency === 'eth') {
    ...
    if (relayerURL) {
        ...
        try {
            ...
            if (netId === 1 || netId === 42 || netId === 5) {
                console.log(`https://${getCurrentNetworkName()}.etherscan.io/tx/${relay.data.txHash}`)
            } else {
        ...
        await tornado.methods.withdraw(proof, ...args).send({ from: senderAccount, value: refund.toString(), gas: 1e6 })
            .on('transactionHash', function (txHash) {
                if (netId === 1 || netId === 42 || netId === 5) {
                    console.log(`https://${getCurrentNetworkName()}.etherscan.io/tx/${txHash}`)
                } else {
...
function calculateFee({ gasPrices, currency, amount, refund, ethPrices, relayerServiceFee, decimals }) {
    ...
    const ethFee = toBN(toWei(prezzo.toString(), 'gwei')).mul(toBN('500000'))
